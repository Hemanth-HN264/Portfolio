...
function Navbar({active,go}){
  ...
  useEffect(()=>{if(!isMobile)so(false);},[isMobile]);
  useEffect(()=>{document.body.style.overflow=open?"hidden":"";return()=>{document.body.style.overflow="";};},[open]);

  const handleNav=(n)=>{go(n);so(false);};

  return(
    <nav style={{background:scrolled||open?"rgba(5,8,16,0.97)":"transparent",backdropFilter:scrolled||open?"blur(20px)":"none",borderBottom:scrolled?"1px solid rgba(16,185,129,0.12)":"none",transition:"all .4s ease",position:"fixed",top:0,left:0,right:0,zIndex:200,padding:scrolled?"14px 0":"22px 0"}}>
      <div style={{maxWidth:1100,margin:"0 auto",padding:"0 20px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <button onClick={()=>handleNav("Home")} style={{fontFamily:"'Clash Display',sans-serif",fontSize:22,fontWeight:800,color:"#fff",background:"none",border:"none",cursor:"pointer",letterSpacing:"-0.5px"}}>
        ...
        {isMobile && (
          <button onClick={()=>so(!open)} aria-label="Toggle menu" style={{background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:10,padding:"0",cursor:"pointer",display:"flex",flexDirection:"column",gap:5,alignItems:"center",justifyContent:"center",width:42,height:42,flexShrink:0}}>
            <span style={{display:"block",width:20,height:2,background:open?"#10b981":"rgba(255,255,255,0.8)",borderRadius:2,transition:"all .3s ease",transform:open?"rotate(45deg) translate(5px,5px)":"none"}}/>
            <span style={{display:"block",width:20,height:2,background:open?"#10b981":"rgba(255,255,255,0.8)",borderRadius:2,transition:"all .3s ease",opacity:open?0:1}}/>
            <span style={{display:"block",width:20,height:2,background:open?"#10b981":"rgba(255,255,255,0.8)",borderRadius:2,transition:"all .3s ease",transform:open?"rotate(-45deg) translate(5px,-5px)":"none"}}/>
          </button>
        )}
      </div>
      ...
      {isMobile && (
        <div style={{maxHeight:open?"500px":"0",overflow:"hidden",transition:"max-height .4s cubic-bezier(.4,0,.2,1)"}}>
          <div style={{display:"flex",flexDirection:"column",padding:"12px 20px 24px",gap:4,borderTop:open?"1px solid rgba(255,255,255,0.07)":"none",marginTop:4}}>
            {NAV.map((n,i)=>(
              <button key={n} onClick={()=>handleNav(n)}
                style={{background:active===n?"rgba(16,185,129,0.15)":"transparent",border:active===n?"1px solid rgba(16,185,129,0.35)":"1px solid transparent",color:active===n?"#10b981":"rgba(255,255,255,0.75)",padding:"13px 18px",borderRadius:14,fontSize:15,fontWeight:600,cursor:"pointer",textAlign:"left",fontFamily:"'DM Sans',sans-serif",transition:`all .2s ease ${i*40}ms`}}>
                {n}
              </button>
            ))}
          </div>
  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",position:"relative",overflow:"hidden",paddingTop:80}}>
      <div style={{position:"absolute",inset:0,pointerEvents:"none"}}>
        <div style={{position:"absolute",top:"15%",left:"5%",width:isMobile?280:560,height:isMobile?280:560,background:"radial-gradient(circle,rgba(16,185,129,0.2) 0%,transparent 68%)",borderRadius:"50%"}}/>
        <div style={{position:"absolute",bottom:"10%",right:"5%",width:isMobile?230:460,height:isMobile?230:460,background:"radial-gradient(circle,rgba(245,158,11,0.16) 0%,transparent 68%)",borderRadius:"50%"}}/>
      ...
      <div style={{maxWidth:1100,margin:"0 auto",padding:"40px 20px",display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr auto",gap:isMobile?32:60,alignItems:"center",position:"relative",zIndex:1,width:"100%"}}>
        <div>
          <div style={anim(0)}>
          ...
          <div style={anim(150)}>
            <h1 style={{fontFamily:"'Clash Display',sans-serif",fontSize:"clamp(42px,6.5vw,84px)",fontWeight:800,color:"#fff",lineHeight:1.0,margin:"0 0 8px",letterSpacing:"-2.5px"}}>
              Hemanth<br/>
          ...
          <div style={anim(280)}>
            <p style={{color:"rgba(255,255,255,0.55)",fontSize:isMobile?15:17,lineHeight:1.8,maxWidth:460,margin:"22px 0 38px",fontFamily:"'DM Sans',sans-serif"}}>
          ...
          <div style={{...anim(400),display:"flex",gap:14,flexWrap:"wrap"}}>
            ...
            onMouseEnter={e=>{e.currentTarget.style.transform="scale(1.06)";e.currentTarget.style.boxShadow="0 0 55px rgba(16,185,129,0.65)";}}
            onMouseLeave={e=>{e.currentTarget.style.transform="scale(1)";e.currentTarget.style.boxShadow="0 0 32px rgba(16,185,129,0.45)";}}>
              View Projects ↗
            </button>
            ...
            onMouseEnter={e=>{e.currentTarget.style.borderColor="rgba(245,158,11,0.55)";e.currentTarget.style.color="#f59e0b";e.currentTarget.style.background="rgba(245,158,11,0.08)";}}
            onMouseLeave={e=>{e.currentTarget.style.borderColor="rgba(255,255,255,0.2)";e.currentTarget.style.color="rgba(255,255,255,0.8)";e.currentTarget.style.background="transparent";}}>
              Get in Touch
            </button>
          </div>
          <div style={{...anim(540),display:"flex",gap:isMobile?24:36,marginTop:50,flexWrap:"wrap"}}>
            {[["2+","Projects","#10b981"],["1","Publication","#f59e0b"],["2","Internships","#a855f7"]].map(([n,l,c])=>(
              <div key={l}>
                <div style={{fontFamily:"'Clash Display',sans-serif",fontSize:isMobile?32:40,fontWeight:800,color:c,lineHeight:1,textShadow:`0 0 24px ${c}88`}}>{n}</div>
        ...
        {!isMobile && (
          <div style={{...anim(320),flexShrink:0}}>
...
function About(){
  const isMobile=useIsMobile();
  return(
    <div style={{minHeight:"100vh",padding:"100px 20px 80px"}}>
      <div style={{maxWidth:1100,margin:"0 auto"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:40}}>
          <Anim dir="left">
            <div style={{background:"linear-gradient(145deg,rgba(16,185,129,0.1),rgba(5,150,105,0.04))",border:"1px solid rgba(16,185,129,0.25)",borderRadius:24,padding:isMobile?24:36,height:"100%",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:20}}>
          {EXP.map((e,i)=>(
            <Anim key={e.company} delay={i*120} dir={i%2===0?"left":"right"}>
              <div style={{background:`linear-gradient(145deg,${e.color}0d,${e.color}05)`,border:`1px solid ${e.color}30`,borderRadius:24,padding:isMobile?20:28,position:"relative",overflow:"hidden",height:"100%"}}>
    ...
    <div style={{minHeight:"100vh",padding:"100px 20px 80px"}}>
      <div style={{maxWidth:1100,margin:"0 auto"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:20}}>
          {Object.entries(SKILLS_DATA).map(([cat,{icon,color,items}],ci)=>(
            <Anim key={cat} delay={ci*100} dir={ci%2===0?"left":"right"}>
              <div style={{background:`linear-gradient(145deg,${color}0d,${color}05)`,border:`1px solid ${color}30`,borderRadius:24,padding:isMobile?20:28,height:"100%",position:"relative",overflow:"hidden"}}>
        ...
        <Anim delay={200} style={{marginTop:40}}>
          <div style={{marginTop:40,background:"linear-gradient(145deg,rgba(245,158,11,0.08),rgba(16,185,129,0.05))",border:"1px solid rgba(245,158,11,0.22)",borderRadius:24,padding:isMobile?20:36}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:isMobile?14:"18px 52px"}}>
    ...
    <div style={{minHeight:"100vh",padding:"100px 20px 80px"}}>
      <div style={{maxWidth:1100,margin:"0 auto"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:28}}>
          {PROJECTS.map((p,i)=>(
            <Anim key={p.title} delay={i*140} dir={i%2===0?"left":"right"}>
              <div style={{background:`linear-gradient(145deg,${p.accent}10,${p.accent}05)`,border:`1px solid ${p.accent}38`,borderRadius:28,padding:isMobile?24:36,position:"relative",overflow:"hidden",transition:"all .4s cubic-bezier(.34,1.56,.64,1)",height:"100%"}}
                ...
                <div style={{position:"relative",zIndex:1}}>
                  ...
                  <h3 style={{fontFamily:"'Clash Display',sans-serif",color:"#fff",fontSize:isMobile?18:22,fontWeight:800,margin:"0 0 6px",letterSpacing:"-0.5px"}}>{p.title}</h3>
        ...
        <Anim delay={200} style={{marginTop:28}}>
          <div style={{marginTop:28,background:"linear-gradient(135deg,rgba(168,85,247,0.1),rgba(16,185,129,0.06))",border:"1px solid rgba(168,85,247,0.28)",borderRadius:28,padding:isMobile?20:36,display:"flex",gap:isMobile?16:24,alignItems:"flex-start",position:"relative",overflow:"hidden",flexDirection:isMobile?"column":"row"}}>
            ...
            <div>
              ...
              <h3 style={{fontFamily:"'Clash Display',sans-serif",color:"#fff",fontSize:isMobile?17:20,fontWeight:800,margin:"12px 0 10px",letterSpacing:"-0.5px"}}>Facial Emotion Recognition for Healthcare AI</h3>
  ...
  return(
    <div style={{minHeight:"100vh",padding:"100px 20px 80px"}}>
      <div style={{maxWidth:1100,margin:"0 auto"}}>
        ...
        <Anim dir="up">
          <div style={{position:"relative",background:"linear-gradient(145deg,rgba(245,158,11,0.1),rgba(16,185,129,0.05))",border:"1px solid rgba(245,158,11,0.28)",borderRadius:32,padding:isMobile?24:48,overflow:"hidden"}}>
            ...
            <h3 style={{fontFamily:"'Clash Display',sans-serif",color:"#fff",fontSize:"clamp(18px,3vw,30px)",fontWeight:800,margin:"0 0 20px",lineHeight:1.3,letterSpacing:"-0.5px",position:"relative",zIndex:1}}>
              Emotion Recognition From EEG Signals Using{isMobile?" ":(<br/>)}
            ...
            <p style={{color:"rgba(255,255,255,0.5)",lineHeight:1.8,fontSize:isMobile?13:15,maxWidth:740,margin:"0 0 36px",fontFamily:"'DM Sans',sans-serif",position:"relative",zIndex:1}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"repeat(3,1fr)",gap:16,marginBottom:32,position:"relative",zIndex:1}}>
              {[["🧠","LSTM Networks","Temporal dependency modeling","#10b981"],["📡","RFFT Features","EEG signal processing","#f59e0b"],["🎯","High Accuracy","Reliable classification","#a855f7"]].map(([ic,t,d,c])=>(
                <div key={t} style={{background:"rgba(255,255,255,0.04)",border:`1px solid ${c}28`,borderRadius:18,padding:isMobile?16:20}}>
        ...
        <Anim delay={200}>
          <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"repeat(3,1fr)",gap:20,marginTop:32}}>
            {[["🏆","Nationally Recognized","Published at NIT Sikkim","#f59e0b"],["🔬","Healthcare AI","Real-world ML applications","#10b981"],["📈","Research Impact","Advancing affective computing","#a855f7"]].map(([ic,t,d,c])=>(
              <div key={t} style={{textAlign:"center",background:`${c}0a`,border:`1px solid ${c}22`,borderRadius:22,padding:isMobile?20:28,transition:"all .3s ease",cursor:"default"}}
  ...
  return(
    <div style={{minHeight:"100vh",padding:"100px 20px 80px"}}>
      <div style={{maxWidth:1100,margin:"0 auto"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1.2fr",gap:40,alignItems:"start"}}>
              ...
              <Anim key={label} delay={i*100} dir="left">
                <div style={{display:"flex",alignItems:"center",gap:16,background:`${c}08`,border:`1px solid ${c}22`,borderRadius:18,padding:20,transition:"all .3s ease",cursor:"default"}}
                  ...
                  <div style={{minWidth:0}}>
                    ...
                    {href?<a href={href} style={{color:"#fff",fontWeight:700,fontSize:isMobile?13:14,fontFamily:"'DM Sans',sans-serif",textDecoration:"none",wordBreak:"break-all"}}>{val}</a>:<div style={{color:"#fff",fontWeight:700,fontSize:14,fontFamily:"'DM Sans',sans-serif"}}>{val}</div>}
                  </div>
                </div>
              </Anim>
            ))}
          ...
          <Anim dir="right" delay={100}>
            {sent?(
              ...
              <div style={{background:"linear-gradient(145deg,rgba(255,255,255,0.04),rgba(255,255,255,0.01))",border:"1px solid rgba(255,255,255,0.08)",borderRadius:28,padding:isMobile?24:36}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:16}}>
                  ...
                  <button onClick={()=>{if(form.name&&form.email&&form.message)ss(true);}} style={{background:"linear-gradient(135deg,#10b981,#059669)",color:"#fff",border:"none",borderRadius:14,padding:15,fontSize:15,fontWeight:700,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",boxShadow:"0 0 32px rgba(16,185,129,0.4)",transition:"all .3s ease",letterSpacing:.3}}
                    onMouseEnter={e=>{e.currentTarget.style.transform="scale(1.03)";e.currentTarget.style.boxShadow="0 0 55px rgba(16,185,129,0.6)";}}
                    onMouseLeave={e=>{e.currentTarget.style.transform="scale(1)";e.currentTarget.style.boxShadow="0 0 32px rgba(16,185,129,0.4)";}}>
                    Send Message →
                  </button>
                </div>
              </div>
